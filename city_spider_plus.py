...
# a = open("列表页失败链接.txt", "r", encoding="utf-8").readlines()
a = open("国家页失败链接.txt", "r", encoding="utf-8").readlines()
...
def get_page_list(country_url):
    ...
    try:
        ...
        if s1.status_code != 200:
            with open("国家页失败链接_temp.txt", "a+", encoding="utf-8") as f:
                f.write(country_url + "\n")
            return "0", "0", "0"
    ...
    try:
        ...
        page_list1 = [country_url] + [part1 + part2 + "oa" + str(i) + "-" + part3 for i in
                                      range(20, int(page_num) * 20, 20)]
    except IndexError as e:
        page_list1 = [country_url]
    cities = []
    range_ = []
    links = []
...
def insert_db(country_url, country):
    ...
    if cities != "0" and range_ != "0":
        ...
        for each in range(0, cities_len):
            ...
            ab = (country, cities[each], range_[each], country_url, links[each])
            try:
...
if __name__ == "__main__":
    ...
    total = {}
    for i in range(lens):
        insert_db(page_list[i], get_name(page_list[i]))
